...
def vacuum_ingest_dir_sync(*, stdout, stderr, days=7, dry_run=False):
    ...
    if not os.path.exists(ingest_dir):
        stdout.write("Ingest directory does not exist: {}".format(ingest_dir))
        return
    ...
    for filename in os.listdir(ingest_dir):
        ...
        try:
            ...
            if file_mtime < cutoff_time:
                ...
                if dry_run:
                    stdout.write("Would remove: {} (age: {:.1f} days)".format(filename, age_days))
                else:
                    os.remove(filepath)
                    stdout.write("Removed: {} (age: {:.1f} days)".format(filename, age_days))
                files_removed += 1
        except OSError as e:
    ...
    stdout.write("  Files processed: {}".format(files_processed))
    stdout.write("  Files {}: {}".format('would be removed' if dry_run else 'removed', files_removed))
    ...
    if unexpected_files:
        stdout.write("  Unexpected files found (skipped): {}".format(len(unexpected_files)))
